...
class MapConverter (object):
    ...
    def map_callback(self, map_msg):
        ...
        contours = self.get_occupied_region(map_array)
        ...
        if mesh_type == "stl":
            ...
            #     mesh.export(f, "stl")
            mesh.export(export_dir+"/map.stl")
            ...
            mesh.export(export_dir+"/map.dae")
...
if __name__ == "__main__":
    ros.init_node("map2gazebo")
    map_topic = ros.get_param("~map_topic", "map")
    occupied_thresh = ros.get_param("~occupied_thresh", 1)
    box_height = ros.get_param("~box_height", 2.0)
    converter = MapConverter(map_topic, threshold=occupied_thresh,
                             height=box_height)
    ros.loginfo("map2gazebo running")
    ros.spin()
